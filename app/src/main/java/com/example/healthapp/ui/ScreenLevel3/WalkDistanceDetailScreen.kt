...
@Composable
fun WalkingDistanceDetailsScreen(
    userId: Int,
    ...
    Scaffold(
        ...
        if (showDialog) {
            AddWalkingDistanceDialog(userId, dailyActivityViewModel) { showDialog = false }
        }
    }
}

@Composable
fun AddWalkingDistanceDialog(
    userId: Int,
    dailyActivityViewModel: DailyActivityViewModel,
    onDismiss: () -> Unit
) {
    val walkingDistance = remember { mutableStateOf("") }
    val dateText = remember { mutableStateOf("") }

    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(text = "添加走路距离") },
        text = {
            Column {
                // 步数输入
                OutlinedTextField(
                    value = walkingDistance.value,
                    onValueChange = { newValue ->
                        // 允许输入数字和一个小数点
                        if (newValue.matches(Regex("^\\d*\\.?\\d*$"))) {
                            walkingDistance.value = newValue
                        }
                    },
                    label = { Text(text = "输入走路距离") },
                    // 控制只弹出数字框
        ...
        confirmButton = {
            Button(
                onClick = {
                    val distance = walkingDistance.value.toDoubleOrNull() ?: 0.0
                    val date =
                        LocalDate.parse(dateText.value, DateTimeFormatter.ISO_DATE).atStartOfDay()
                    dailyActivityViewModel.addOrUpdateWalkDistance(userId, date, distance)
                    onDismiss()
                }) {
                Text(text = "确认")
            }
...
@Composable
fun WalkingDistanceRow(walkingDistance: Double, date: String) {
    Card(
        modifier = Modifier
